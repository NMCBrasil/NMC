...
if uploaded_file is None:
    ...
    tempo_medio = 0
    if relatorio_tipo == "enterprise":
        # Detecta colunas de datas/hora de abertura e fechamento
    ...
    if relatorio_tipo == "consumer":
        grafico_com_tabela(df_filtrado, "Satélite", "Satélite", "🛰")

    # ---------------- DOWNLOAD COMPLETO ----------------
    def export_dashboard_html():
        html = "<html><head><meta charset='utf-8'><title>{}</title></head><body>".format(titulo_dashboard)
        html += "<h2>{}</h2>".format(titulo_dashboard)
        # Adiciona métricas
        html += f"<p>⏱ Tempo médio total (min): {tempo_medio:.2f}</p>"
        html += f"<p>📌 Maior ofensor: {maior_ofensor}</p>"
        html += f"<p>📊 % dos chamados do maior ofensor: {pct_maior:.2f}%</p>"
        # Função para gerar tabela + gráfico
        def tabela_graf_html(df_graf, coluna, titulo, icone="📁"):
            df_graf_clean = df_graf[df_graf[coluna] != "Não informado"]
            if df_graf_clean.empty: return ""
            tabela = df_graf_clean.groupby(coluna).size().reset_index(name="Qtd")
            tabela["%"] = (tabela["Qtd"]/tabela["Qtd"].sum()*100).round(2)
            tabela_html = tabela.to_html(index=False)
            fig = px.bar(tabela, x=coluna, y="Qtd", text="Qtd", color="Qtd",
                         color_continuous_scale="Blues", template="plotly_white")
            fig_html = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
            return f"<div style='display:flex; gap:30px; margin-bottom:50px;'><div style='flex:1'>{tabela_html}</div><div style='flex:2'>{fig_html}</div></div>"
        # Adiciona gráficos principais
        html += tabela_graf_html(df_filtrado, "Criado por", "Chamados abertos por usuário", "🔵")
        html += tabela_graf_html(df_fechados, col_fechado, "Chamados fechados por usuário", "🔴")
        if relatorio_tipo == "enterprise":
            html += tabela_graf_html(df_filtrado, "Reclamação", "Reclamação", "📌")
        html += tabela_graf_html(df_filtrado, col_diag, col_diag, "📌")
        if relatorio_tipo == "consumer":
            html += tabela_graf_html(df_filtrado, "Satélite", "Satélite", "🛰")
        html += "</body></html>"
